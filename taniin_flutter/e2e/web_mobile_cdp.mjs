...
try {
  ...
  console.log(`Mobile E2E MetaMask text count: ${metaMaskCount}`);
  if (metaMaskCount > 0) {
  ...
  await page.screenshot({ path: 'test-results/web-mobile-cdp.png' });

  if (errors.length > 0) {
    throw new Error(`Browser errors: ${errors.join(' | ')}`);
  }

  console.log(`E2E web mobile movement passed at ${baseUrl}`);
} finally {
  await browser.close();
}
